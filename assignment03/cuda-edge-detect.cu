...
/**
 * GPU kernel for edge detection using sobel operator
 */
#define OUT_TILE_DIM 14
#define IN_TILE_DIM (OUT_TILE_DIM + 2) // Input is 2 pixels bigger than output
#define CF 2 // coarse factor

__global__ 
void gpu_edge_detect_kernel(unsigned char *in, unsigned char *out, 
    int width, int height, int threshold) 
{
    int ty = threadIdx.y;
    int tx = threadIdx.x;

    int row = blockIdx.y * OUT_TILE_DIM + ty;
    int col = blockIdx.x * OUT_TILE_DIM * CF + tx;

    __shared__ unsigned char in_s[IN_TILE_DIM][IN_TILE_DIM];

    for (int c = 0; c < CF; c++) {
        if (row >= 0 && row < height && col >= 0 && col + c*OUT_TILE_DIM < width)
            in_s[ty][tx] = in[row*width + col + c*OUT_TILE_DIM];

        __syncthreads();

        if (ty >= 1 && ty < IN_TILE_DIM - 1 && tx >= 1 && tx < IN_TILE_DIM - 1) {
            if (row >= 0 && row < height && col >= 0 && col + c*OUT_TILE_DIM < width) {
                if (row >= 1 && col >= 1 && row < height-1 && col + c*OUT_TILE_DIM < width-1) {
                    const int Gx = 
                        in_s[ty-1][tx-1] - in_s[ty-1][tx+1]
                        + 2*in_s[ty][tx-1] - 2*in_s[ty][tx+1]
                        + in_s[ty+1][tx-1] - in_s[ty+1][tx+1];
                    
                    const int Gy =
                        in_s[ty-1][tx-1] + 2*in_s[ty-1][tx] + in_s[ty-1][tx+1]
                        - in_s[ty+1][tx-1] - 2*in_s[ty+1][tx] - in_s[ty+1][tx+1];

                    const int magnitude = Gx * Gx + Gy * Gy;
                    if  (magnitude > threshold*threshold)
                        out[row*width+col + c*OUT_TILE_DIM] = WHITE;
                    else
                        out[row*width+col + c*OUT_TILE_DIM] = BLACK;
                }
                else {
                    out[row*width+col + c*OUT_TILE_DIM] = BLACK;
                }
...
double gpu_edge_detect(const PGM_image *in, PGM_image *edges, int threshold) 
{
    ...
    cudaMemcpy(in_d, in->bmap, size, cudaMemcpyHostToDevice);
    ...
    gpu_edge_detect_kernel<<<gridDim, blockDim>>>(
        in_d, out_d, width, height, threshold);

    cudaDeviceSynchronize(); 
    cudaCheckError();

    cudaMemcpy(edges->bmap, out_d, size, cudaMemcpyDeviceToHost);
...
int main( int argc, char* argv[] )
{
    ...
    const double gpuTime = gpu_edge_detect(&bmap, &out, threshold);
    fprintf(stderr, "Execution time %.6f\n", gpuTime);
